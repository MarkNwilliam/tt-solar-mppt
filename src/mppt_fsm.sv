...
//   1. P = V_pv * I_pv over the top PVB bits of each 10-bit sample (a /4096
//      "power signature"; coarse but the P&O only needs the direction of dP,
//      and it keeps the multiplier tiny for the 1x1 tile).
...
module mppt_fsm #(
    ...
    parameter HYST       = 16'd1        // in 8-bit scaled-power units (~ /4096)
) (
    input  wire               clk,
    input  wire               rst_n,
    input  wire               en,
    ...
    localparam int PVB = 4;                   // power-signature bits (top of VW)
    localparam int PW  = 2 * PVB;             // power width (8)

...
//   1. P = V_pv * I_pv over the top PVB bits of each 10-bit sample (a /2048
...
module mppt_fsm #(
    ...
    parameter HYST       = 16'd1        // in 10-bit scaled-power units (~ /2048)
) (
    input  wire               clk,
    input  wire               rst_n,
    input  wire               en,
    ...
    localparam int PVB = 5;                   // power-signature bits (top of VW)
    localparam int PW  = 2 * PVB;             // power width (10)

// telemetry.sv — UART TX of a 2-byte status frame (8N1, LSB-first bits)
// Wire order (byte 0 first): vpv[11:4] | vbat[11:4].
// Samples carry their top 8 bits (78 mV resolution on the 20 V / 20 A scale).
// There is no sync byte: the receiver re-syncs on the idle gap enforced by
// tx_busy. PV/battery currents and the switcher duty / charge stage
// {chg[2:0]} / fault latch {flt[4:0]} are deliberately NOT in the frame —
// MPPT perturbation is visible as duty motion, and the rest is already
// exposed live on uio_out (DUTY[5:0]) and uo_out (LED0/LED1/FAULT), so a
// 2-byte frame keeps the snapshot register small and only sends what is not
// visible on pins.

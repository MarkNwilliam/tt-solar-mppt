...
`vpv[11:4] | vbat[11:4]` (each sample carries its top 8 bits; no sync byte — re-sync on the tx-busy idle gap). Currents, duty,
charge stage and fault are exposed live on `DUTY[5:0]` and the
`LED0`/`LED1`/`FAULT` pins instead.
